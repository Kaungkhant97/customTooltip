...
class TooltipTextDrawable extends Drawable {
  ...
  private Tooltips.Direction gravity;

  public TooltipTextDrawable(final Context context, final Tooltips.Builder builder) {


    this.ellipseSize =  4;
    final int strokeWidth = 2;
    final int backgroundColor =  1;
    final int strokeColor = 1;
  ...
  public void setAnchor(final Tooltips.Direction gravity, int padding, @Nullable Point point) {
  ...
  private void calculatePathWithGravity(final Rect outBounds, final int left, final int top,
      final int right, final int bottom, final float maxY, final float maxX, final float minY,
      final float minX) {
    boolean drawPoint =
        isDrawPoint(left, top, right, bottom, maxY, maxX, minY, minX, tmpPoint, point, gravity,
            arrowWeight);
    ...
    if (drawPoint && gravity == Tooltips.Direction.BOTTOM) {
    ...
    if (drawPoint && gravity == Tooltips.Direction.LEFT) {
    ...
    if (drawPoint && gravity == Tooltips.Direction.TOP) {
    ...
    if (drawPoint && gravity == Tooltips.Direction.RIGHT) {
  ...
  private static boolean isDrawPoint(final int left, final int top, final int right,
      final int bottom, final float maxY, final float maxX, final float minY, final float minX,
      final Point tmpPoint, final Point point, final Tooltips.Direction gravity,
      final int arrowWeight) {
    boolean drawPoint = false;
    tmpPoint.set(point.x, point.y);

    if (gravity == Tooltips.Direction.RIGHT || gravity == Tooltips.Direction.LEFT) {
  ...
  private static void clampPoint(final int left, final int top, final int right, final int bottom,
  ...
  @TargetApi(Build.VERSION_CODES.LOLLIPOP) @Override public void getOutline(Outline outline) {
